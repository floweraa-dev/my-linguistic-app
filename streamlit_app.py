...
# Загрузка необходимых словарей для работы TextBlob
@st.cache_resource
def download_nltk_resources():
    try:
        ...
        st.error(f"Error downloading linguistic resources: {e}")

download_nltk_resources()

# Чтобы результаты определения языка были стабильными
...
if st.button("Analyze Text"):
    if user_text:
        ...
        if lang == 'en':
            ...
            # Теперь blob.words будет работать без ошибок
